...
if __name__ == '__main__':
    ...
    particle_df = pd.read_csv(particle_file,names=['Id','x','y','z','Rot_x','Rot_y','Rot_z','R','E_k_p','Mat-id','F_x','F_y','F_z'])
    ...
    contact_df = pd.read_csv(contact_file, names=['P_1','P_2','n_x','n_y','n_z','h_','F_','F_b','F_p','h_max','F_Tx','F_Ty','F_Tz','F_Tbx','F_Tby','F_Tbz','F_Tpx','F_Tpy','F_Tpz','Binder contact','b_t'])
    ...
    bins = np.arange(start=0, stop=2, step=0.00859)
    hist, bin_edges=np.histogram(hist_vec,bins=bins)
    print(np.sum(hist))

    fig,ax = plt.subplots()
    lns = ax.hist(hist_vec,bins=bins,density=False)
    lns_yield_point = ax.plot([0.00859,0.00859],[0,1.1*max(hist)],label='Yield overlap')
