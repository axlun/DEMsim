#
# Created by Axel on 2025-05-13
# Adapted from article_4/in_plane_stress_spreads.py and article_3/in_plane_stress_spreads.py
#
...
def rve_stress_calculator(periodic_BC_data, force_fabric_tensor_data, layer_height_data):
    ...
    return time, vol, sxx, syy, szz, tau_xy, tau_xz, tau_yx, tau_yz, tau_zx, tau_zy


def in_plane_stress_processing(sim_case, no_sims, bc=''):
    simulation_dictionary = {}
    for i in range(1, no_sims + 1):
        sim_dir = '/scratch/users/axlun/DEMsim/results/article_4/final_runs/' + str(i) + '/' + sim_case + \
                  '/electrode_long_cycling' + bc
        print(sim_dir)
    ...
    return time, sxx_mean, sxx_std, sxx_matrix, \
...
class Cycling:
    def __init__(self, sim_dir):
        ...
        self.time, self.rve_vol, self.sxx, self.syy, self.szz, self.tau_xy, self.tau_xz, self.tau_yx, \
...
class Simulation:
    def __init__(self, sim_case, no_sims, label='template', bc=''):
        self.time, self.sxx_mean, self.sxx_std, self.sxx_matrix, \
        ...
        self.height_mean, self.height_std, self.height_matrix \
            = in_plane_stress_processing(sim_case, no_sims, bc)
...
if __name__ == '__main__':
    ...
    # ==FIGURE SAVE DIR=================================================================================================
    fig_dir = 'C:/temp/figures/article_4/final_runs/cycling_spreads/'
    try:
    ...
    end_time = ref.time[-1]-1
    n_cycles = 3
    n_soc_ticks = n_cycles * 2 + 1
    tick_array = np.linspace(0,end_time,n_soc_ticks)
    soc_tick_labels = [0, 100, 0, 100, 0, 100, 0]
    ########################################PARTICLE FRACTURE###########################################################
    # =FIGURE PARTICLE PRESSURE=========================================================================================
    fig_particle_pressure, ax_particle_pressure = plt.subplots()
    ax_particle_pressure.plot(ref.time-1,
                              ref.in_plane_stress_mean * 1E-6,
                              'k-', zorder=1)
    ax_particle_pressure.fill_between(
        ref.time-1,
        (ref.in_plane_stress_mean - ref.in_plane_stress_std) * 1E-6,
        (ref.in_plane_stress_mean + ref.in_plane_stress_std) * 1E-6,
        color='C0', label=ref.label, alpha=1, zorder=1)

    ax_particle_pressure.plot(sf_2e8.time-1,
                              sf_2e8.in_plane_stress_mean * 1E-6,
                              'k-', zorder=2)
    ax_particle_pressure.fill_between(
        sf_2e8.time-1,
        (sf_2e8.in_plane_stress_mean - sf_2e8.in_plane_stress_std) * 1E-6,
        (sf_2e8.in_plane_stress_mean + sf_2e8.in_plane_stress_std) * 1E-6,
        color='C1', label=sf_2e8.label, alpha=1, zorder=2)

    ax_particle_pressure.plot(sf_1e8.time-1,
                              sf_1e8.in_plane_stress_mean * 1E-6,
                              'k-', zorder=3)
    ax_particle_pressure.fill_between(
        sf_1e8.time-1,
        (sf_1e8.in_plane_stress_mean - sf_1e8.in_plane_stress_std) * 1E-6,
        (sf_1e8.in_plane_stress_mean + sf_1e8.in_plane_stress_std) * 1E-6,
        color='C2', label=sf_1e8.label, alpha=1, zorder=3)

    ax_particle_pressure.plot(sf_5e7.time-1,
                              sf_5e7.in_plane_stress_mean * 1E-6,
                              'k-', zorder=4)
    ax_particle_pressure.fill_between(
        sf_5e7.time-1,
        (sf_5e7.in_plane_stress_mean - sf_5e7.in_plane_stress_std) * 1E-6,
        (sf_5e7.in_plane_stress_mean + sf_5e7.in_plane_stress_std) * 1E-6,
        color='C3', label=sf_5e7.label, alpha=1, zorder=4)
    ax_particle_pressure.set_xlim(xmin=0, xmax=end_time)
    ax_particle_pressure.set_ylim(ymin=0, ymax=175)
    ax_particle_pressure.yaxis.set_major_locator(MultipleLocator(25))
    ax_particle_pressure.set_ylabel('In-plane compressive stress [MPa]')
    ax_particle_pressure.set_xlabel('SOC [%]')
    ax_particle_pressure.set_xticks(tick_array)
    ax_particle_pressure.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_pressure.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ...
    ax_particle_pressure_split.set_xlabel('SOC [%]')
    ax_particle_pressure_split.set_ylabel('In-plane compressive stress [MPa]')
    # ax_particle_pressure_split.set_xlim(xmin=0)
    # ax_particle_pressure_split.set_ylim(ymin=0, ymax=180)

    ax_particle_pressure_split.plot(ref.time-1, ref.in_plane_stress_matrix.T * 1E-6,
                                    color='C0', label=ref.label)

    ax_particle_pressure_split.plot(sf_5e7.time-1, sf_5e7.in_plane_stress_matrix.T * 1E-6,
                                    color='C1', label=sf_5e7.label)

    ax_particle_pressure_split.plot(sf_1e8.time-1, sf_1e8.in_plane_stress_matrix.T * 1E-6,
                                    color='C2', label=sf_1e8.label)

    ax_particle_pressure_split.plot(sf_2e8.time-1, sf_2e8.in_plane_stress_matrix.T * 1E-6,
    ...
    ax_particle_contacts_p.plot(ref.time-1,
    ...
    ax_particle_contacts_p.plot(sf_2e8.time-1,
    ...
    ax_particle_contacts_p.plot(sf_1e8.time-1,
    ...
    ax_particle_contacts_p.plot(sf_5e7.time-1,
    ...
    ax_particle_contacts_p.set_xlim(xmin=0, xmax=end_time)
    ax_particle_contacts_p.set_ylim(ymin=2500, ymax=6500)
    ax_particle_contacts_p.yaxis.set_major_locator(MultipleLocator(500))
    ax_particle_contacts_p.set_ylabel('Particle contacts [-]')
    ax_particle_contacts_p.set_xlabel('SOC [%]')
    ax_particle_contacts_p.legend(loc='best')
    ax_particle_contacts_p.set_xticks(tick_array)
    ax_particle_contacts_p.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_contacts_p.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_contacts_p_split.set_xlabel('SOC [%]')
    ...
    ax_particle_contacts_p_split.plot(ref.time-1, ref.particle_contacts_matrix.T,
                                      color='C0', label=ref.label)

    ax_particle_contacts_p_split.plot(sf_5e7.time-1, sf_5e7.particle_contacts_matrix.T,
                                      color='C1', label=sf_5e7.label)

    ax_particle_contacts_p_split.plot(sf_1e8.time-1, sf_1e8.particle_contacts_matrix.T,
                                      color='C2', label=sf_1e8.label)

    ax_particle_contacts_p_split.plot(sf_2e8.time-1, sf_2e8.particle_contacts_matrix.T,
                                      color='C3', label=sf_2e8.label)
    ...
    ax_particle_contacts_b.plot(ref.time-1,
    ...
    ax_particle_contacts_b.plot(sf_2e8.time-1,
    ...
    ax_particle_contacts_b.plot(sf_1e8.time-1,
    ...
    ax_particle_contacts_b.plot(sf_5e7.time-1,
    ...
    ax_particle_contacts_b.set_xlim(xmin=0, xmax=end_time)
    ax_particle_contacts_b.set_ylim(ymin=9700, ymax=10000)
    ax_particle_contacts_b.yaxis.set_major_locator(MultipleLocator(50))
    ax_particle_contacts_b.set_ylabel('Binder contacts [-]')
    ax_particle_contacts_b.set_xlabel('SOC [%]')

    ax_particle_contacts_b.set_xticks(tick_array)
    ax_particle_contacts_b.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_contacts_b.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_contacts_b_split.set_xlabel('SOC [%]')
    ax_particle_contacts_b_split.set_ylabel('Binder contacts [-]')
    ax_particle_contacts_b_split.plot(ref.time-1, ref.binder_contacts_matrix.T,
                                      color='C0', label=ref.label)

    ax_particle_contacts_b_split.plot(sf_5e7.time-1, sf_5e7.binder_contacts_matrix.T,
                                      color='C1', label=sf_5e7.label)
    ax_particle_contacts_b_split.plot(sf_1e8.time-1, sf_1e8.binder_contacts_matrix.T,
                                      color='C2', label=sf_1e8.label)

    ax_particle_contacts_b_split.plot(sf_2e8.time-1, sf_2e8.binder_contacts_matrix.T,
                                      color='C3', label=sf_2e8.label)
    ...
    ax_particle_fracture_p.plot(sf_2e8.time-1,
    ...
    ax_particle_fracture_p.plot(sf_1e8.time-1,
    ...
    ax_particle_fracture_p.plot(sf_5e7.time-1,
    ...
    ax_particle_fracture_p.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_fracture_p.set_xlabel('SOC [%]')
    ax_particle_fracture_p.legend(loc='best')

    ax_particle_fracture_p.set_xticks(tick_array)
    ax_particle_fracture_p.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_fracture_p.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_fracture_p_split.set_xlabel('SOC [%]')
    ax_particle_fracture_p_split.set_ylabel('Fraction of fractured particles [-]')
    # ax_particle_fracture_p_split.set_xlim(xmin=0)
    # ax_particle_contacts_p_split.set_ylim(ymin=0, ymax=180)
    ax_particle_fracture_p_split.plot(sf_5e7.time-1, sf_5e7.fractured_particles_matrix.T,
                                      color='C1', label=sf_5e7.label)
    ax_particle_fracture_p_split.plot(sf_1e8.time-1, sf_1e8.fractured_particles_matrix.T,
                                      color='C2', label=sf_1e8.label)

    ax_particle_fracture_p_split.plot(sf_2e8.time-1, sf_2e8.fractured_particles_matrix.T,
                                      color='C3', label=sf_2e8.label)
    ...
    ax_particle_fracture_b.plot(sf_2e8.time-1,
                                sf_2e8.fractured_binders_mean * 1E2,
                                'k-', zorder=1)
    ax_particle_fracture_b.fill_between(sf_2e8.time-1,
                                        (sf_2e8.fractured_binders_mean - sf_2e8.fractured_binders_std) * 1E2,
                                        (sf_2e8.fractured_binders_mean + sf_2e8.fractured_binders_std) * 1E2,
                                        color='C1', label=sf_2e8.label, alpha=1, zorder=1)

    ax_particle_fracture_b.plot(sf_1e8.time-1,
                                sf_1e8.fractured_binders_mean * 1E2,
                                'k-', zorder=2)
    ax_particle_fracture_b.fill_between(sf_1e8.time-1,
                                        (sf_1e8.fractured_binders_mean - sf_1e8.fractured_binders_std) * 1E2,
                                        (sf_1e8.fractured_binders_mean + sf_1e8.fractured_binders_std) * 1E2,
                                        color='C2', label=sf_1e8.label, alpha=1, zorder=2)

    ax_particle_fracture_b.plot(sf_5e7.time-1,
                                sf_5e7.fractured_binders_mean * 1E2,
                                'k-', zorder=3)
    ax_particle_fracture_b.fill_between(sf_5e7.time-1,
                                        (sf_5e7.fractured_binders_mean - sf_5e7.fractured_binders_std) * 1E2,
                                        (sf_5e7.fractured_binders_mean + sf_5e7.fractured_binders_std) * 1E2,
                                        color='C3', label=sf_5e7.label, alpha=1, zorder=3)

    ax_particle_fracture_b.set_xlim(xmin=0, xmax=end_time)
    ax_particle_fracture_b.set_ylim(ymin=0)
    ax_particle_fracture_b.set_ylabel('Fractured binder contacts [%]')
    ax_particle_fracture_b.set_xlabel('SOC [%]')

    ax_particle_fracture_b.set_xticks(tick_array)
    ax_particle_fracture_b.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_fracture_b.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_fracture_b_split.set_xlabel('SOC [%]')
    ...
    ax_particle_fracture_b_split.plot(sf_5e7.time-1, sf_5e7.fractured_binders_matrix.T,
                                      color='C1', label=sf_5e7.label)

    ax_particle_fracture_b_split.plot(sf_1e8.time-1, sf_1e8.fractured_binders_matrix.T,
                                      color='C2', label=sf_1e8.label)

    ax_particle_fracture_b_split.plot(sf_2e8.time-1, sf_2e8.fractured_binders_matrix.T,
                                      color='C3', label=sf_2e8.label)
    ...
    ax_binder_pressure.plot(ref.time-1,
                            ref.in_plane_stress_mean * 1E-6,
                            'k-', zorder=1)
    ...
    ax_binder_pressure.plot(ef_2e_1.time-1,
                            ef_2e_1.in_plane_stress_mean * 1E-6,
                            'k-', zorder=2)
    ax_binder_pressure.fill_between(
        ef_2e_1.time-1,
        (ef_2e_1.in_plane_stress_mean - ef_2e_1.in_plane_stress_std) * 1E-6,
        (ef_2e_1.in_plane_stress_mean + ef_2e_1.in_plane_stress_std) * 1E-6,
        color='C1', label=ef_2e_1.label, alpha=1, zorder=2)

    ax_binder_pressure.plot(ef_1e_1.time-1,
                            ef_1e_1.in_plane_stress_mean * 1E-6,
                            'k-', zorder=3)
    ax_binder_pressure.fill_between(
        ef_1e_1.time-1,
        (ef_1e_1.in_plane_stress_mean - ef_1e_1.in_plane_stress_std) * 1E-6,
        (ef_1e_1.in_plane_stress_mean + ef_1e_1.in_plane_stress_std) * 1E-6,
        color='C2', label=ef_1e_1.label, alpha=1, zorder=3)

    ax_binder_pressure.plot(ef_5e_2.time-1,
                            ef_5e_2.in_plane_stress_mean * 1E-6,
                            'k-', zorder=4)
    ax_binder_pressure.fill_between(
        ef_5e_2.time-1,
        (ef_5e_2.in_plane_stress_mean - ef_5e_2.in_plane_stress_std) * 1E-6,
        (ef_5e_2.in_plane_stress_mean + ef_5e_2.in_plane_stress_std) * 1E-6,
        color='C3', label=ef_5e_2.label, alpha=1, zorder=4)
    ax_binder_pressure.set_xlim(xmin=0, xmax=end_time)
    ax_binder_pressure.set_ylim(ymin=-5, ymax=175)
    ax_binder_pressure.yaxis.set_major_locator(MultipleLocator(25))
    ax_binder_pressure.set_ylabel('In-plane compressive stress [MPa]')
    ax_binder_pressure.set_xlabel('SOC [%]')

    ax_binder_pressure.set_xticks(tick_array)
    ax_binder_pressure.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_binder_pressure.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_binder_pressure_split.set_xlabel('SOC [%]')
    ax_binder_pressure_split.set_ylabel('In-plane compressive stress [MPa]')
    # ax_binder_pressure_split.set_xlim(xmin=0)
    # ax_binder_pressure_split.set_ylim(ymin=0, ymax=180)

    ax_binder_pressure_split.plot(ref.time-1, ref.in_plane_stress_matrix.T * 1E-6,
                                  color='C0', label=ref.label)

    ax_binder_pressure_split.plot(ef_5e_2.time-1, ef_5e_2.in_plane_stress_matrix.T * 1E-6,
                                  color='C1', label=ef_5e_2.label)

    ax_binder_pressure_split.plot(ef_1e_1.time-1, ef_1e_1.in_plane_stress_matrix.T * 1E-6,
                                  color='C2', label=ef_1e_1.label)

    ax_binder_pressure_split.plot(ef_2e_1.time-1, ef_2e_1.in_plane_stress_matrix.T * 1E-6,
    ...
    ax_binder_contacts_p.plot(ref.time-1,
                              ref.particle_contacts_mean,
                              'k-', zorder=1)
    ...
    ax_binder_contacts_p.plot(ef_2e_1.time-1,
    ...
    ax_binder_contacts_p.plot(ef_1e_1.time-1,
    ...
    ax_binder_contacts_p.plot(ef_5e_2.time-1,
    ...
    ax_binder_contacts_p.set_xlim(xmin=0, xmax=end_time)
    ax_binder_contacts_p.set_ylim(ymin=0, ymax=6500)
    ax_binder_contacts_p.yaxis.set_major_locator(MultipleLocator(500))
    ax_binder_contacts_p.set_ylabel('Particle contacts [-]')
    ax_binder_contacts_p.set_xlabel('SOC [%]')
    ax_binder_contacts_p.legend(loc='best')

    ax_binder_contacts_p.set_xticks(tick_array)
    ax_binder_contacts_p.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_binder_contacts_p.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_binder_contacts_p_split.set_xlabel('SOC [%]')
    ...
    ax_binder_contacts_p_split.plot(ref.time-1, ref.particle_contacts_matrix.T,
                                    color='C0', label=ref.label)

    ax_binder_contacts_p_split.plot(ef_5e_2.time-1, ef_5e_2.particle_contacts_matrix.T,
                                    color='C1', label=ef_5e_2.label)

    ax_binder_contacts_p_split.plot(ef_1e_1.time-1, ef_1e_1.particle_contacts_matrix.T,
                                    color='C2', label=ef_1e_1.label)

    ax_binder_contacts_p_split.plot(ef_2e_1.time-1, ef_2e_1.particle_contacts_matrix.T,
                                    color='C3', label=ef_2e_1.label)
    ...
    ax_binder_contacts_b.plot(ref.time-1,
                              ref.binder_contacts_mean,
                              'k-', zorder=1)
    ...
    ax_binder_contacts_b.plot(ef_2e_1.time-1,
    ...
    ax_binder_contacts_b.plot(ef_1e_1.time-1,
    ...
    ax_binder_contacts_b.plot(ef_5e_2.time-1,
    ...
    ax_binder_contacts_b.set_xlim(xmin=0, xmax=end_time)
    ax_binder_contacts_b.set_ylim(ymin=8000, ymax=10000)
    ax_binder_contacts_b.yaxis.set_major_locator(MultipleLocator(200))
    ax_binder_contacts_b.set_ylabel('Binder contacts [-]')
    ax_binder_contacts_b.set_xlabel('SOC [%]')

    ax_binder_contacts_b.set_xticks(tick_array)
    ax_binder_contacts_b.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_binder_contacts_b.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_binder_contacts_b_split.set_xlabel('SOC [%]')
    ...
    ax_binder_contacts_b_split.plot(ref.time-1, ref.binder_contacts_matrix.T,
                                    color='C0', label=ref.label)

    ax_binder_contacts_b_split.plot(ef_5e_2.time-1, ef_5e_2.binder_contacts_matrix.T,
                                    color='C1', label=ef_5e_2.label)
    ax_binder_contacts_b_split.plot(ef_1e_1.time-1, ef_1e_1.binder_contacts_matrix.T,
                                    color='C2', label=ef_1e_1.label)

    ax_binder_contacts_b_split.plot(ef_2e_1.time-1, ef_2e_1.binder_contacts_matrix.T,
                                    color='C3', label=ef_2e_1.label)
    ...
    ax_binder_fracture_p.plot(ef_2e_1.time-1,
    ...
    ax_binder_fracture_p.plot(ef_1e_1.time-1,
    ...
    ax_binder_fracture_p.plot(ef_5e_2.time-1,
    ...
    ax_binder_fracture_p.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_binder_fracture_p.set_xlabel('SOC [%]')

    ax_binder_fracture_p.set_xticks(tick_array)
    ax_binder_fracture_p.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_binder_fracture_p.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_binder_fracture_p_split.set_xlabel('SOC [%]')
    ax_binder_fracture_p_split.set_ylabel('Fraction of fractured particles [-]')
    # ax_binder_fracture_p_split.set_xlim(xmin=0)
    # ax_binder_contacts_p_split.set_ylim(ymin=0, ymax=180)
    ax_binder_fracture_p_split.plot(ef_5e_2.time-1, ef_5e_2.fractured_particles_matrix.T,
                                    color='C1', label=ef_5e_2.label)
    ax_binder_fracture_p_split.plot(ef_1e_1.time-1, ef_1e_1.fractured_particles_matrix.T,
                                    color='C2', label=ef_1e_1.label)

    ax_binder_fracture_p_split.plot(ef_2e_1.time-1, ef_2e_1.fractured_particles_matrix.T,
                                    color='C3', label=ef_2e_1.label)
    ...
    ax_binder_fracture_b.plot(ef_2e_1.time-1,
    ...
    ax_binder_fracture_b.plot(ef_1e_1.time-1,
    ...
    ax_binder_fracture_b.plot(ef_5e_2.time-1,
    ...
    ax_binder_fracture_b.set_xlim(xmin=0, xmax=end_time)
    ax_binder_fracture_b.set_ylim(ymin=0, ymax=40)
    ax_binder_fracture_b.yaxis.set_major_locator(MultipleLocator(5))
    ax_binder_fracture_b.set_ylabel('Fractured binder contacts [%]')
    ax_binder_fracture_b.set_xlabel('SOC [%]')

    ax_binder_fracture_b.set_xticks(tick_array)
    ax_binder_fracture_b.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_binder_fracture_b.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_binder_fracture_b_split.set_xlabel('SOC [%]')
    ...
    ax_binder_fracture_b_split.plot(ef_5e_2.time-1, ef_5e_2.fractured_binders_matrix.T,
                                    color='C1', label=ef_5e_2.label)

    ax_binder_fracture_b_split.plot(ef_1e_1.time-1, ef_1e_1.fractured_binders_matrix.T,
                                    color='C2', label=ef_1e_1.label)

    ax_binder_fracture_b_split.plot(ef_2e_1.time-1, ef_2e_1.fractured_binders_matrix.T,
                                    color='C3', label=ef_2e_1.label)
    ...
    sf_2e8_ef_1e_1_comp = Simulation('sf_2e8_ef_1e-1', no_sims, r'$\varepsilon=-2$ %', '_e_-2')
    sf_2e8_ef_1e_1_ten = Simulation('sf_2e8_ef_1e-1', no_sims, r'$\varepsilon=2$ %', '_e_2')

    # =FIGURE BINDER PARTICLE PRESSURE==================================================================================
    fig_particle_binder_pressure, ax_particle_binder_pressure = plt.subplots()
    ax_particle_binder_pressure.plot(sf_2e8_ef_1e_1.time-1,
                                     -sf_2e8_ef_1e_1.sxx_mean* 1E-6,
                                     'k-', zorder=1)
    ax_particle_binder_pressure.fill_between(
        sf_2e8_ef_1e_1.time-1,
        -(sf_2e8_ef_1e_1.sxx_mean - sf_2e8_ef_1e_1.sxx_std) * 1E-6,
        -(sf_2e8_ef_1e_1.sxx_mean + sf_2e8_ef_1e_1.sxx_std) * 1E-6,
        color='C0', label=r'$\sigma_{xx}$', alpha=1, zorder=1)

    ax_particle_binder_pressure.plot(sf_2e8_ef_1e_1.time-1,
                                     -sf_2e8_ef_1e_1.syy_mean* 1E-6,
                                     'k-', zorder=2)
    ax_particle_binder_pressure.fill_between(
        sf_2e8_ef_1e_1.time-1,
        -(sf_2e8_ef_1e_1.syy_mean - sf_2e8_ef_1e_1.syy_std) * 1E-6,
        -(sf_2e8_ef_1e_1.syy_mean + sf_2e8_ef_1e_1.syy_std) * 1E-6,
        color='C1', label=r'$\sigma_{yy}$', alpha=1, zorder=2)

    ax_particle_binder_pressure.plot(sf_2e8_ef_1e_1.time-1,
                                     -sf_2e8_ef_1e_1.sxy_mean* 1E-6,
                                     'k-', zorder=3)
    ax_particle_binder_pressure.fill_between(
        sf_2e8_ef_1e_1.time-1,
        -(sf_2e8_ef_1e_1.sxy_mean - sf_2e8_ef_1e_1.sxy_std) * 1E-6,
        -(sf_2e8_ef_1e_1.sxy_mean + sf_2e8_ef_1e_1.sxy_std) * 1E-6,
        color='C2', label=r'$\tau_{xy}$', alpha=1, zorder=3)

    ax_particle_binder_pressure.set_xticks(tick_array)
    ax_particle_binder_pressure.set_xticklabels(soc_tick_labels)
    ax_particle_binder_pressure.set_xlim(xmin=0, xmax=end_time)
    ax_particle_binder_pressure.set_ylim(ymin=-10, ymax=100)
    ax_particle_binder_pressure.yaxis.set_major_locator(MultipleLocator(10))
    ax_particle_binder_pressure.set_ylabel('In-plane compressive stress components [MPa]')
    ax_particle_binder_pressure.set_xlabel('SOC [%]')

    ax_2nd_x = ax_particle_binder_pressure.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_binder_pressure_split.set_xlabel('Time [s]')
    ax_particle_binder_pressure_split.set_ylabel('In-plane compressive stress [MPa]')

    ax_particle_binder_pressure_split.plot(sf_2e8_ef_1e_1.time-1, -sf_2e8_ef_1e_1.sxx_matrix.T * 1E-6,
                                           color='C0', label='sxx')

    ax_particle_binder_pressure_split.plot(sf_2e8_ef_1e_1.time-1,
                                           -sf_2e8_ef_1e_1.syy_matrix.T * 1E-6,
                                           color='C1', label='syy')

    ax_particle_binder_pressure_split.plot(sf_2e8_ef_1e_1.time-1,
                                           -sf_2e8_ef_1e_1.sxy_matrix.T * 1E-6,
                                           color='C2', label='txy')
    ...
    # =FIGURE BINDER PARTICLE PRESSURE COMP=============================================================================
    fig_particle_binder_pressure_comp, ax_particle_binder_pressure_comp = plt.subplots()
    ax_particle_binder_pressure_comp.plot(sf_2e8_ef_1e_1_comp.time-2,
                                     -sf_2e8_ef_1e_1_comp.sxx_mean* 1E-6,
                                     'k-', zorder=1)
    ax_particle_binder_pressure_comp.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        -(sf_2e8_ef_1e_1_comp.sxx_mean - sf_2e8_ef_1e_1_comp.sxx_std) * 1E-6,
        -(sf_2e8_ef_1e_1_comp.sxx_mean + sf_2e8_ef_1e_1_comp.sxx_std) * 1E-6,
        color='C0', label=r'$\sigma_{xx}$', alpha=1, zorder=1)

    ax_particle_binder_pressure_comp.plot(sf_2e8_ef_1e_1_comp.time-2,
                                     -sf_2e8_ef_1e_1_comp.syy_mean* 1E-6,
                                     'k-', zorder=2)
    ax_particle_binder_pressure_comp.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        -(sf_2e8_ef_1e_1_comp.syy_mean - sf_2e8_ef_1e_1_comp.syy_std) * 1E-6,
        -(sf_2e8_ef_1e_1_comp.syy_mean + sf_2e8_ef_1e_1_comp.syy_std) * 1E-6,
        color='C1', label=r'$\sigma_{yy}$', alpha=1, zorder=2)

    ax_particle_binder_pressure_comp.plot(sf_2e8_ef_1e_1_comp.time-2,
                                     -sf_2e8_ef_1e_1_comp.sxy_mean* 1E-6,
                                     'k-', zorder=3)
    ax_particle_binder_pressure_comp.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        -(sf_2e8_ef_1e_1_comp.sxy_mean - sf_2e8_ef_1e_1_comp.sxy_std) * 1E-6,
        -(sf_2e8_ef_1e_1_comp.sxy_mean + sf_2e8_ef_1e_1_comp.sxy_std) * 1E-6,
        color='C2', label=r'$\tau_{xy}$', alpha=1, zorder=3)

    ax_particle_binder_pressure_comp.set_xticks(tick_array)
    ax_particle_binder_pressure_comp.set_xticklabels(soc_tick_labels)
    ax_particle_binder_pressure_comp.set_xlim(xmin=0, xmax=end_time)
    ax_particle_binder_pressure_comp.set_ylim(ymin=-10, ymax=100)
    ax_particle_binder_pressure_comp.yaxis.set_major_locator(MultipleLocator(10))
    ax_particle_binder_pressure_comp.set_ylabel('In-plane compressive stress components [MPa]')
    ax_particle_binder_pressure_comp.set_xlabel('SOC [%]')

    ax_2nd_x = ax_particle_binder_pressure_comp.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    fig_particle_binder_pressure_comp.tight_layout()
    ax_particle_binder_pressure_comp.legend(loc='best')
    fname = fig_dir + 'particle_binder_pressure_comp.svg'
    plt.savefig(fname)

    # =BINDER PARTICLE PRESSURE COMP SPLIT==============================================================================
    fig_particle_binder_pressure_comp_split, ax_particle_binder_pressure_comp_split = plt.subplots()
    ax_particle_binder_pressure_comp_split.set_xlabel('Time [s]')
    ax_particle_binder_pressure_comp_split.set_ylabel('In-plane compressive stress [MPa]')

    ax_particle_binder_pressure_comp_split.plot(sf_2e8_ef_1e_1_comp.time-2, -sf_2e8_ef_1e_1_comp.sxx_matrix.T * 1E-6,
                                           color='C0', label='sxx')

    ax_particle_binder_pressure_comp_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                           -sf_2e8_ef_1e_1_comp.syy_matrix.T * 1E-6,
                                           color='C1', label='syy')

    ax_particle_binder_pressure_comp_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                           -sf_2e8_ef_1e_1_comp.sxy_matrix.T * 1E-6,
                                           color='C2', label='txy')

    ax_particle_binder_pressure_comp_split.legend(loc='best')
    fig_particle_binder_pressure_comp_split.tight_layout()
    fname = fig_dir + 'particle_binder_pressure_comp_split.svg'
    plt.savefig(fname)

    # =FIGURE BINDER PARTICLE PRESSURE TEN==============================================================================
    fig_particle_binder_pressure_ten, ax_particle_binder_pressure_ten = plt.subplots()
    ax_particle_binder_pressure_ten.plot(sf_2e8_ef_1e_1_ten.time-2,
                                          -sf_2e8_ef_1e_1_ten.sxx_mean* 1E-6,
                                          'k-', zorder=1)
    ax_particle_binder_pressure_ten.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        -(sf_2e8_ef_1e_1_ten.sxx_mean - sf_2e8_ef_1e_1_ten.sxx_std) * 1E-6,
        -(sf_2e8_ef_1e_1_ten.sxx_mean + sf_2e8_ef_1e_1_ten.sxx_std) * 1E-6,
        color='C0', label=r'$\sigma_{xx}$', alpha=1, zorder=1)

    ax_particle_binder_pressure_ten.plot(sf_2e8_ef_1e_1_ten.time-2,
                                          -sf_2e8_ef_1e_1_ten.syy_mean* 1E-6,
                                          'k-', zorder=2)
    ax_particle_binder_pressure_ten.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        -(sf_2e8_ef_1e_1_ten.syy_mean - sf_2e8_ef_1e_1_ten.syy_std) * 1E-6,
        -(sf_2e8_ef_1e_1_ten.syy_mean + sf_2e8_ef_1e_1_ten.syy_std) * 1E-6,
        color='C1', label=r'$\sigma_{yy}$', alpha=1, zorder=2)

    ax_particle_binder_pressure_ten.plot(sf_2e8_ef_1e_1_ten.time-2,
                                          -sf_2e8_ef_1e_1_ten.sxy_mean* 1E-6,
                                          'k-', zorder=3)
    ax_particle_binder_pressure_ten.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        -(sf_2e8_ef_1e_1_ten.sxy_mean - sf_2e8_ef_1e_1_ten.sxy_std) * 1E-6,
        -(sf_2e8_ef_1e_1_ten.sxy_mean + sf_2e8_ef_1e_1_ten.sxy_std) * 1E-6,
        color='C2', label=r'$\tau_{xy}$', alpha=1, zorder=3)

    ax_particle_binder_pressure_ten.set_xticks(tick_array)
    ax_particle_binder_pressure_ten.set_xticklabels(soc_tick_labels)
    ax_particle_binder_pressure_ten.set_xlim(xmin=0, xmax=end_time)
    ax_particle_binder_pressure_ten.set_ylim(ymin=-10, ymax=100)
    ax_particle_binder_pressure_ten.yaxis.set_major_locator(MultipleLocator(10))
    ax_particle_binder_pressure_ten.set_ylabel('In-plane compressive stress components [MPa]')
    ax_particle_binder_pressure_ten.set_xlabel('SOC [%]')

    ax_2nd_x = ax_particle_binder_pressure_ten.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    fig_particle_binder_pressure_ten.tight_layout()
    ax_particle_binder_pressure_ten.legend(loc='best')
    fname = fig_dir + 'particle_binder_pressure_ten.svg'
    plt.savefig(fname)

    # =BINDER PARTICLE PRESSURE TEN SPLIT===============================================================================
    fig_particle_binder_pressure_ten_split, ax_particle_binder_pressure_ten_split = plt.subplots()
    ax_particle_binder_pressure_ten_split.set_xlabel('Time [s]')
    ax_particle_binder_pressure_ten_split.set_ylabel('In-plane compressive stress [MPa]')

    ax_particle_binder_pressure_ten_split.plot(sf_2e8_ef_1e_1_ten.time-2, -sf_2e8_ef_1e_1_ten.sxx_matrix.T * 1E-6,
                                                color='C0', label='sxx')

    ax_particle_binder_pressure_ten_split.plot(sf_2e8_ef_1e_1_ten.time-2,
                                                -sf_2e8_ef_1e_1_ten.syy_matrix.T * 1E-6,
                                                color='C1', label='syy')

    ax_particle_binder_pressure_ten_split.plot(sf_2e8_ef_1e_1_ten.time-2,
                                                -sf_2e8_ef_1e_1_ten.sxy_matrix.T * 1E-6,
                                                color='C2', label='txy')

    ax_particle_binder_pressure_ten_split.legend(loc='best')
    fig_particle_binder_pressure_ten_split.tight_layout()
    fname = fig_dir + 'particle_binder_pressure_ten_split.svg'
    plt.savefig(fname)
    ...
    ax_particle_binder_contacts_p.plot(sf_2e8_ef_1e_1_ten.time-2,
                                       sf_2e8_ef_1e_1_ten.particle_contacts_mean,
                                       'k-', zorder=1)
    ax_particle_binder_contacts_p.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        (sf_2e8_ef_1e_1_ten.particle_contacts_mean - sf_2e8_ef_1e_1_ten.particle_contacts_std),
        (sf_2e8_ef_1e_1_ten.particle_contacts_mean + sf_2e8_ef_1e_1_ten.particle_contacts_std),
        color='C1', label=sf_2e8_ef_1e_1_ten.label, alpha=1, zorder=1)

    ax_particle_binder_contacts_p.plot(sf_2e8_ef_1e_1.time-1,
    ...
    ax_particle_binder_contacts_p.plot(sf_2e8_ef_1e_1_comp.time-2,
                                       sf_2e8_ef_1e_1_comp.particle_contacts_mean,
                                       'k-', zorder=3)
    ax_particle_binder_contacts_p.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        (sf_2e8_ef_1e_1_comp.particle_contacts_mean - sf_2e8_ef_1e_1_comp.particle_contacts_std),
        (sf_2e8_ef_1e_1_comp.particle_contacts_mean + sf_2e8_ef_1e_1_comp.particle_contacts_std),
        color='C3', label=sf_2e8_ef_1e_1_comp.label, alpha=1, zorder=3)

    ax_particle_binder_contacts_p.set_xlim(xmin=0,xmax=end_time)
    ...
    ax_particle_binder_contacts_p.set_xlabel('SOC [%]')

    ax_particle_binder_contacts_p.set_xticks(tick_array)
    ax_particle_binder_contacts_p.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_binder_contacts_p.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0], linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_binder_contacts_p_split.set_xlabel('SOC [%]')
    ...
    ax_particle_binder_contacts_p_split.plot(sf_2e8_ef_1e_1_ten.time-2, sf_2e8_ef_1e_1_ten.particle_contacts_matrix.T,
                                             color='C0', label=sf_2e8_ef_1e_1_ten.label)

    ax_particle_binder_contacts_p_split.plot(sf_2e8_ef_1e_1.time-1,
                                             sf_2e8_ef_1e_1.particle_contacts_matrix.T,
                                             color='C1', label=sf_2e8_ef_1e_1.label)
    ax_particle_binder_contacts_p_split.plot(sf_2e8_ef_1e_1.time-1,
                                             sf_2e8_ef_1e_1.particle_contacts_matrix.T, color='C1')

    ax_particle_binder_contacts_p_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                             sf_2e8_ef_1e_1_comp.particle_contacts_matrix.T,
                                             color='C2', label=sf_2e8_ef_1e_1_comp.label)
    ax_particle_binder_contacts_p_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                             sf_2e8_ef_1e_1_comp.particle_contacts_matrix.T, color='C2')
    ...
    ax_particle_binder_contacts_b.plot(sf_2e8_ef_1e_1_ten.time-2,
                                       sf_2e8_ef_1e_1_ten.binder_contacts_mean,
                                       'k-', zorder=1)
    ax_particle_binder_contacts_b.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        (sf_2e8_ef_1e_1_ten.binder_contacts_mean - sf_2e8_ef_1e_1_ten.binder_contacts_std),
        (sf_2e8_ef_1e_1_ten.binder_contacts_mean + sf_2e8_ef_1e_1_ten.binder_contacts_std),
        color='C1', label=sf_2e8_ef_1e_1_ten.label, alpha=1, zorder=1)

    ax_particle_binder_contacts_b.plot(sf_2e8_ef_1e_1.time-1,
    ...
    ax_particle_binder_contacts_b.plot(sf_2e8_ef_1e_1_comp.time-2,
                                       sf_2e8_ef_1e_1_comp.binder_contacts_mean,
                                       'k-', zorder=3)
    ax_particle_binder_contacts_b.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        (sf_2e8_ef_1e_1_comp.binder_contacts_mean - sf_2e8_ef_1e_1_comp.binder_contacts_std),
        (sf_2e8_ef_1e_1_comp.binder_contacts_mean + sf_2e8_ef_1e_1_comp.binder_contacts_std),
        color='C3', label=sf_2e8_ef_1e_1_comp.label,alpha=1, zorder=3)

    ax_particle_binder_contacts_b.set_xlim(xmin=0,xmax=end_time)
    ...
    ax_particle_binder_contacts_b.set_xlabel('SOC [%]')

    ax_particle_binder_contacts_b.set_xticks(tick_array)
    ax_particle_binder_contacts_b.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_binder_contacts_b.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_binder_contacts_b_split.set_xlabel('SOC [%]')
    ...
    ax_particle_binder_contacts_b_split.plot(sf_2e8_ef_1e_1_ten.time-2, sf_2e8_ef_1e_1_ten.binder_contacts_matrix.T,
                                             color='C0', label=sf_2e8_ef_1e_1_ten.label)

    ax_particle_binder_contacts_b_split.plot(sf_2e8_ef_1e_1.time-1,
                                             sf_2e8_ef_1e_1.binder_contacts_matrix.T,
                                             color='C1', label=sf_2e8_ef_1e_1.label)

    ax_particle_binder_contacts_b_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                             sf_2e8_ef_1e_1_comp.binder_contacts_matrix.T,
                                             color='C2', label=sf_2e8_ef_1e_1_comp.label)
    ...
    ax_particle_binder_fracture_p.plot(sf_2e8_ef_1e_1_ten.time-2,
                                       sf_2e8_ef_1e_1_ten.fractured_particles_mean * 1E2,
                                       'k-', zorder=1)
    ax_particle_binder_fracture_p.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        (sf_2e8_ef_1e_1_ten.fractured_particles_mean - sf_2e8_ef_1e_1_ten.fractured_particles_std) * 1E2,
        (sf_2e8_ef_1e_1_ten.fractured_particles_mean + sf_2e8_ef_1e_1_ten.fractured_particles_std) * 1E2,
        color='C1', label=sf_2e8_ef_1e_1_ten.label, alpha=1, zorder=1)

    ax_particle_binder_fracture_p.plot(sf_2e8_ef_1e_1.time-1,
                                       sf_2e8_ef_1e_1.fractured_particles_mean * 1E2,
                                       'k-', zorder=2)
    ax_particle_binder_fracture_p.fill_between(
        sf_2e8_ef_1e_1.time-1,
        (sf_2e8_ef_1e_1.fractured_particles_mean - sf_2e8_ef_1e_1.fractured_particles_std) * 1E2,
        (sf_2e8_ef_1e_1.fractured_particles_mean + sf_2e8_ef_1e_1.fractured_particles_std) * 1E2,
        color='C2', label=sf_2e8_ef_1e_1.label, alpha=1, zorder=2)

    ax_particle_binder_fracture_p.plot(sf_2e8_ef_1e_1_comp.time-2,
                                       sf_2e8_ef_1e_1_comp.fractured_particles_mean * 1E2,
                                       'k-', zorder=3)
    ax_particle_binder_fracture_p.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        (sf_2e8_ef_1e_1_comp.fractured_particles_mean - sf_2e8_ef_1e_1_comp.fractured_particles_std) * 1E2,
        (sf_2e8_ef_1e_1_comp.fractured_particles_mean + sf_2e8_ef_1e_1_comp.fractured_particles_std) * 1E2,
        color='C3', label=sf_2e8_ef_1e_1_comp.label, alpha=1, zorder=3)

    ax_particle_binder_fracture_p.set_xlim(xmin=0,xmax=end_time)
    ax_particle_binder_fracture_p.xaxis.set_major_locator(MultipleLocator(5))
    ax_particle_binder_fracture_p.set_ylim(ymin=0, ymax=100)
    ...
    ax_particle_binder_fracture_p.set_xlabel('SOC [%]')

    ax_particle_binder_fracture_p.set_xticks(tick_array)
    ax_particle_binder_fracture_p.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_binder_fracture_p.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_binder_fracture_p_split.set_xlabel('SOC [%]')
    ...
    # ax_particle_binder_contacts_p_split.set_ylim(ymin=0, ymax=180)
    ax_particle_binder_fracture_p_split.plot(sf_2e8_ef_1e_1_ten.time-2,
                                             sf_2e8_ef_1e_1_ten.fractured_particles_matrix.T,
                                             color='C0', label=sf_2e8_ef_1e_1_ten.label)

    ax_particle_binder_fracture_p_split.plot(sf_2e8_ef_1e_1.time-1,
                                             sf_2e8_ef_1e_1.fractured_particles_matrix.T,
                                             color='C1', label=sf_2e8_ef_1e_1.label)

    ax_particle_binder_fracture_p_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                             sf_2e8_ef_1e_1_comp.fractured_particles_matrix.T,
                                             color='C2', label=sf_2e8_ef_1e_1_comp.label)
    ...
    ax_particle_binder_fracture_b.plot(sf_2e8_ef_1e_1_ten.time-2,
                                       sf_2e8_ef_1e_1_ten.fractured_binders_mean * 1E2,
                                       'k-', zorder=1)
    ax_particle_binder_fracture_b.fill_between(
        sf_2e8_ef_1e_1_ten.time-2,
        (sf_2e8_ef_1e_1_ten.fractured_binders_mean - sf_2e8_ef_1e_1_ten.fractured_binders_std) * 1E2,
        (sf_2e8_ef_1e_1_ten.fractured_binders_mean + sf_2e8_ef_1e_1_ten.fractured_binders_std) * 1E2,
        color='C1', label=sf_2e8_ef_1e_1_ten.label, alpha=1, zorder=1)

    ax_particle_binder_fracture_b.plot(sf_2e8_ef_1e_1.time-1,
                                       sf_2e8_ef_1e_1.fractured_binders_mean * 1E2,
                                       'k-', zorder=2)
    ax_particle_binder_fracture_b.fill_between(
        sf_2e8_ef_1e_1.time-1,
        (sf_2e8_ef_1e_1.fractured_binders_mean - sf_2e8_ef_1e_1.fractured_binders_std) * 1E2,
        (sf_2e8_ef_1e_1.fractured_binders_mean + sf_2e8_ef_1e_1.fractured_binders_std) * 1E2,
        color='C2', label=sf_2e8_ef_1e_1.label, alpha=1, zorder=2)

    ax_particle_binder_fracture_b.plot(sf_2e8_ef_1e_1_comp.time-2,
                                       sf_2e8_ef_1e_1_comp.fractured_binders_mean * 1E2,
                                       'k-', zorder=3)
    ax_particle_binder_fracture_b.fill_between(
        sf_2e8_ef_1e_1_comp.time-2,
        (sf_2e8_ef_1e_1_comp.fractured_binders_mean - sf_2e8_ef_1e_1_comp.fractured_binders_std) * 1E2,
        (sf_2e8_ef_1e_1_comp.fractured_binders_mean + sf_2e8_ef_1e_1_comp.fractured_binders_std) * 1E2,
        color='C3', label=sf_2e8_ef_1e_1_comp.label, alpha=1, zorder=3)

    ax_particle_binder_fracture_b.set_xlim(xmin=0,xmax=end_time)
    ax_particle_binder_fracture_b.xaxis.set_major_locator(MultipleLocator(5))
    ax_particle_binder_fracture_b.set_ylim(ymin=0, ymax=14)
    ax_particle_binder_fracture_b.yaxis.set_major_locator(MultipleLocator(2))
    ax_particle_binder_fracture_b.set_ylabel('Fractured binder contacts [%]')
    ax_particle_binder_fracture_b.set_xlabel('SOC [%]')

    ax_particle_binder_fracture_b.set_xticks(tick_array)
    ax_particle_binder_fracture_b.set_xticklabels(soc_tick_labels)

    ax_2nd_x = ax_particle_binder_fracture_b.twiny()
    ax_2nd_x.plot([0, end_time], [0, 0],linewidth=0)
    ax_2nd_x.set_xticks(np.linspace(0, end_time, n_cycles + 1))
    ax_2nd_x.set_xticklabels(np.arange(0, n_cycles + 1, 1))
    ax_2nd_x.set_xlim(xmin=0, xmax=end_time)
    ...
    ax_particle_binder_fracture_b_split.set_xlabel('SOC [%]')
    ...
    ax_particle_binder_fracture_b_split.plot(sf_2e8_ef_1e_1_ten.time-2,
                                             sf_2e8_ef_1e_1_ten.fractured_binders_matrix.T,
                                             color='C0', label=sf_2e8_ef_1e_1_ten.label)

    ax_particle_binder_fracture_b_split.plot(sf_2e8_ef_1e_1.time-1,
                                             sf_2e8_ef_1e_1.fractured_binders_matrix.T,
                                             color='C1', label=sf_2e8_ef_1e_1.label)

    ax_particle_binder_fracture_b_split.plot(sf_2e8_ef_1e_1_comp.time-2,
                                             sf_2e8_ef_1e_1_comp.fractured_binders_matrix.T,
                                             color='C2', label=sf_2e8_ef_1e_1_comp.label)
