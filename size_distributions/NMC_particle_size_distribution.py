...
if __name__ == '__main__':
    ...
    lns_q3_log = ax_q3_log.plot(particle_diameter_exponent_experimental_laser_diffraction, q_3_experimental_laser_diffraction, label='q3')
    ...
    q3_lin_experimental_laser_diffraction = q_3_experimental_laser_diffraction / (np.log(10) * 10 ** particle_diameter_exponent_experimental_laser_diffraction)
    q_3_lin_experimental_tomography = q_3_experimental_tomography / (np.log(10) * 10 ** particle_diameter_exponent_experimental_tomography)
    # print(q3_lin)
    ...
    [shape_q_3_opt, scale_q_3_opt], pcov = optimize.curve_fit(
        ...
        p0=(shape_q3_guess, scale_q3_guess),  # initial value of the parameters
    )
    ...
    (Ebner_shape,Ebner_loc,Ebner_scale) = stats.gamma.fit(Ebner_particle_size)
    print("Ebner fit: " + str((Ebner_shape,Ebner_loc,Ebner_scale)))

    # Ebner_size_array = np.linspace(min(Ebner_particle_size),max(Ebner_particle_size),100)
    Ebner_size_array = np.linspace(0,60,10000)

    Ebner_pdf = stats.gamma.pdf(Ebner_size_array,Ebner_shape,Ebner_loc,Ebner_scale)

    ebner_q3_pdf = Ebner_size_array ** 3 *Ebner_pdf * len(Ebner_particle_size)/np.sum(Ebner_particle_size**3)
    ...
    lns_Ebner_pdf = ax_ebner_fit_hist.plot(Ebner_size_array,Ebner_pdf,label='PDF')
    ...
    lns_Ebner = ax_q0_q3_ebner.plot(Ebner_size_array, Ebner_pdf,label='Ebner')
    ...
    Q_3_NMC622_experimental = np.array([0.01, 10, 50, 90, 100]) / 100
    ...
    shape_Q_3_NMC622_guess = 10
    scale_Q_3_NMC622_guess = 1

    Q_3_NMC622_guess = f_gamma_cdf(D_NMC622_array, shape_Q_3_NMC622_guess, scale_Q_3_NMC622_guess)

    [shape_Q_3_NMC622_opt, scale_Q_3_NMC622_opt], pcov = optimize.curve_fit(
        f=f_gamma_cdf,  # model function
        xdata=D_NMC622_experimental,  # x data
        ydata=Q_3_NMC622_experimental,  # y data
        p0=(shape_Q_3_NMC622_guess, scale_Q_3_NMC622_guess),  # initial value of the parameters
    )
    Q_3_NMC622 = f_gamma_cdf(D_NMC622_array, shape_Q_3_NMC622_opt, scale_Q_3_NMC622_opt)
    q_3_NMC622 = f_gamma_pdf(D_NMC622_array, shape_Q_3_NMC622_opt, scale_Q_3_NMC622_opt)

    # print(popt)
    print('Gamma parameters for Q_3_NMC622\n' + 'shape_Q_3_NMC622 = ' + str(
        shape_Q_3_NMC622_opt) + '\nscale_Q_3_NMC622 = '
          + str(scale_Q_3_NMC622_opt))

    fig_Q_3_NMC622, ax_Q_3_NMC622 = plt.subplots()
    ...
    lns_Q_3_NMC622_fit = ax_Q_3_NMC622.plot(D_NMC622_array, Q_3_NMC622, label=r'$Q_3$ fit')
    ax_Q_3_NMC622.set_title('Cumulative distribution Q3 of NMC622')
    ax_Q_3_NMC622.legend(loc='best')

    fig_q_3_NMC622, ax_q_3_NMC622 = plt.subplots()
    # lns_q_3_NMC622_experimental = ax_Q_3_NMC622.plot(D_NMC622_experimental, Q_3_NMC622_experimental, marker='X',
    #                                                  markersize=16, linewidth=0, label='Experimental points')
    # lns_q_3_NMC622_guess = ax_q_3_NMC622.plot(D_NMC622_array, q_3_NMC622_guess, label=r'$Q_3$ guess')
    lns_q_3_NMC622_fit = ax_q_3_NMC622.plot(D_NMC622_array, q_3_NMC622, label=r'$q_3$')
    ax_q_3_NMC622.set_title('Probability distribution function q3 of NMC622')
    ax_q_3_NMC622.legend(loc='best')
