...
//    auto p2 = simulator.create_particle(radius,Vec3{2*radius+(mat->binder_thickness_fraction*radius)*1.001 ,0 , 0},Vec3{0,0,0}, mat);
    ...
    auto p1 = simulator.create_particle(radius,Vec3{-radius-(mat->binder_thickness_fraction*radius)*1.010/2.0,0 , 0},Vec3{0,0,0}, mat);
    auto p2 = simulator.create_particle(radius,Vec3{radius+(mat->binder_thickness_fraction*radius)*1.010/2.0,0 , 0},Vec3{0,0,0}, mat);
...
//=====Time controlled=================================================================================================================
    ...
    auto particle_tangential_displacement =  radius*mat->binder_thickness_fraction*0.01; //Move particles 1% of the binder distance
    auto particle_velocity = particle_tangential_displacement/2.0/move_time.count();
    ...
    auto normal_vel = normal_move_distance/2.0/move_time.count();
...
//Tangential movement
    ...
    for (double n : num_array) {
        ...
        p2->set_velocity(Vec3{0,-particle_velocity*n,0});
        RunForTime.reset(move_time);
        simulator.run(RunForTime);
...
//
////    Diagonal movement
//    p1->set_velocity(Vec3{0,-particle_velocity,particle_velocity});
