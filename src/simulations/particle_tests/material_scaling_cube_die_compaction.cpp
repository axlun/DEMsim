//
// Created by Axel on 2024-10-15
// based on swelling_cube_die_compaction.cpp
//
...
void DEM::material_scaling_cube_die_compaction(const std::string& settings_file_name) {
    using namespace DEM;
    ...
    auto material_scaling_rate = parameters.get_parameter<double>("material_scaling_rate");
    std::cout << "material_scaling_rate = " << material_scaling_rate << std::endl;
...
//    std::chrono::duration<double> unloading_time {parameters.get_parameter<double>("unloading_time")};
    ...
    auto particle_positions = random_fill_box(-box_side/2, box_side/2, -box_side/2, box_side/2,
                                              -box_side/2, box_side/2, particle_radii, max_radii); //, material->bt); what material parameter is this?
    ...
    bool restart = parameters.get_parameter<bool>("restart");
        ...
        // calculating the distance required for prescribed density levels and giving each surface a velocity to reach a
    // certain density
    for (unsigned level = 0; level != density_levels.size(); ++level)
    {
        ...
        if (restart)
        {
            ...
            unload_top_surface->set_velocity(Vec3(0, 0, -surface_velocity));
            auto unload_bottom_surface =
                    unloading_simulator.get_surface<EngineType::PointSurfacePointer>("bottom_surface");
            unload_bottom_surface->set_velocity(Vec3(0, 0, surface_velocity));
            auto unload_front_surface =
                    unloading_simulator.get_surface<EngineType::PointSurfacePointer>("front_surface");
            unload_front_surface->set_velocity(Vec3(0, surface_velocity, 0));
            auto unload_back_surface =
                    unloading_simulator.get_surface<EngineType::PointSurfacePointer>("back_surface");
            unload_back_surface->set_velocity(Vec3(0, -surface_velocity, 0));
            auto unload_left_surface =
                    unloading_simulator.get_surface<EngineType::PointSurfacePointer>("left_surface");
            unload_left_surface->set_velocity(Vec3(surface_velocity, 0, 0));
            auto unload_right_surface =
                    unloading_simulator.get_surface<EngineType::PointSurfacePointer>("right_surface");
            unload_right_surface->set_velocity(Vec3(-surface_velocity, 0, 0));
