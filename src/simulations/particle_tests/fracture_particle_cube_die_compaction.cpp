//
// Created by Axel on 2025-01-14
// based on material_scaling_cube_die_compaction.cpp
//
...
void DEM::fracturing_particle_cube_die_compaction(const std::string& settings_file_name) {
    using namespace DEM;
    ...
    std::cout << "Particle_file is: " << particle_file << std::endl;
...
//    std::cout << particle_radii.size() << std::endl;
//    particle_radii.assign(N,*particle_radii.begin());
    ...
    for (unsigned level = 0; level != density_levels.size(); ++level)
    {
        ...
        if (restart)
        {
            ...
            EngineType::RunForTime time_to_unload(unloading_simulator, simulation_time/2);
            std::cout << "before running unload sim" << std::endl;
            unloading_simulator.run(time_to_unload);
        }
        else
        {
            run_for_time.reset(simulation_time/2);
            simulator.run(run_for_time);
        }
    }
}