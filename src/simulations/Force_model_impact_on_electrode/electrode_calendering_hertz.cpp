...
void DEM::electrode_calendering_hertz(const std::string& settings_file_name) {
    using namespace DEM;
...
//    box_height = 1; //testing box height
    ...
    auto particle_positions = random_fill_box(-.75*box_side / 2, .75*box_side / 2, -.75*box_side / 2, .75*box_side / 2,
...
//    }
    ...
    auto filling_output = simulator.create_output(output_directory , 1E-3s);
...
//new comment
    ...
    simulator.set_mass_scale_factor(1E2); //Orig 1E2
...
// New packing method
    ...
    EngineType::RunForTime run_for_time(simulator, .1E-2s);//Gives particles velocity of .1m/s
    simulator.run(run_for_time);

    simulator.set_gravity(Vec3(0, 0, -1E-1)); //Use gravity for initial packing of particles
    EngineType::RunForTime run_for_time_2(simulator, 3E-2s);
